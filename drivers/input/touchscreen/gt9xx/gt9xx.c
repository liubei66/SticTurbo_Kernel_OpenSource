...
#define GOODIX_INPUT_EVENT_START			0
#define GOODIX_INPUT_EVENT_SENSITIVE_MODE_OFF		0
#define GOODIX_INPUT_EVENT_SENSITIVE_MODE_ON		1
#define GOODIX_INPUT_EVENT_STYLUS_MODE_OFF		2
#define GOODIX_INPUT_EVENT_STYLUS_MODE_ON		3
#define GOODIX_INPUT_EVENT_WAKUP_MODE_OFF		4
#define GOODIX_INPUT_EVENT_WAKUP_MODE_ON		5
#define GOODIX_INPUT_EVENT_COVER_MODE_OFF		6
#define GOODIX_INPUT_EVENT_COVER_MODE_ON		7
#define GOODIX_INPUT_EVENT_END				7
		  ...
		  u8 *key_value)
{
	...
	u8 point_data[2 + 1 + 8 * GTP_MAX_TOUCH_ID + 1] = {GTP_READ_COOR_ADDR >> 8, GTP_READ_COOR_ADDR & 0xFF};
...
static void gtp_work_func(struct goodix_ts_data *ts)
{
	...
	if (test_bit(PANEL_RESETTING, &ts->flags)) {
		dev_info(&ts->client->dev, "panel is resetting\n");
		return;
	}

	if (!test_bit(WORK_THREAD_ENABLED, &ts->flags)) {
		dev_info(&ts->client->dev, "work thread is not enabled\n");
		return;
	}
	...
	if (!point_state) {
		dev_info(&ts->client->dev, "Invalid finger points\n");
		return;
	}
...
static int gtp_get_lockdown_info(struct goodix_ts_data *ts)
{
	int ret = -1;

	ret = gtp_i2c_read_dbl_check(ts->client, GTP_REG_LOCKDOWN_INFO, ts->lockdown_info, GTP_LOCKDOWN_INFO_SIZE);
	if (ret == SUCCESS)
		dev_info(&ts->client->dev,
...
static int gtp_find_valid_cfg_data(struct goodix_ts_data *ts)
{
	...
	char cfg_name[22];
	struct goodix_config_data *cfg = &ts->pdata->config;
	struct goodix_cfg_info *config_info = ts->pdata->cfg_info;
...
#endif
	/* read sensor id */
	sensor_id = ts->lockdown_info[0];
	/* parse config data */
#ifdef CONFIG_OF
	for (i = 0; i < ts->pdata->cfg_size; i++, config_info++) {
			if (sensor_id == config_info->vendor) {
				dev_info(&ts->client->dev, "vendor = 0x%x found it!\n", ts->lockdown_info[0]);
				ts->current_index = i;
				break;
			} else {
				continue;
	...
	if (i == ts->pdata->cfg_size) {
		if (!ts->pdata->use_default_cfg) {
			...
			dev_info(&ts->client->dev, "don't find cfg %s, use default id:0x%x\n", cfg_name, ts->pdata->default_sensor_id);
			snprintf(cfg_name, sizeof(cfg_name), "goodix,cfg-group-0x%x", ts->pdata->default_sensor_id);
			ts->current_index = 0;
		}
	} else
		snprintf(cfg_name, sizeof(cfg_name), "%s", config_info->cfg_name);
	dev_info(&ts->client->dev, "Get config data from device tree, cfg_name:%s\n", cfg_name);
				    ...
				    size_t size, loff_t *ppos)
{
	int i = 0, cnt = 0;
	char *ptr = NULL;
	char temp_data[GTP_CONFIG_MAX_LENGTH + 2] = {(u8)(GTP_REG_CONFIG_DATA >> 8), (u8)GTP_REG_CONFIG_DATA};
	...
	cnt += snprintf(ptr, 50, "==== GT9XX config init value====\n");
	ptr += cnt;
	for (i = 0 ; i < GTP_CONFIG_MAX_LENGTH ; i++) {
		cnt += snprintf(ptr + cnt, 10, "0x%02X ", cfg->data[i + 2]);

		if (i % 8 == 7)
			cnt += snprintf(ptr + cnt, 10, "\n");
	}

	cnt += snprintf(ptr + cnt, 10, "\n");
	cnt += snprintf(ptr + cnt, 50, "==== GT9XX config real value====\n");
	gtp_i2c_read(i2c_connect_client, temp_data, GTP_CONFIG_MAX_LENGTH + 2);

	for (i = 0 ; i < GTP_CONFIG_MAX_LENGTH ; i++) {
		cnt += snprintf(ptr + cnt, 10, "0x%02X ", temp_data[i + 2]);
		if (i % 8 == 7)
			cnt += snprintf(ptr + cnt, 10, "\n");
	}
	*ppos += cnt;
	if (copy_to_user(page, ptr, cnt) != 0) {
		kfree(ptr);
		ptr = NULL;
		return 0;
	} else {
...
static ssize_t gtp_fw_version_read(struct file *file, char __user *buf, size_t count, loff_t *pos)
{
	int cnt = 0, ret = 0;
	...
	if (*pos != 0)
		return 0;
	ret = gtp_i2c_read_dbl_check(ts->client, GTP_REG_CONFIG_DATA, &cfg_ver, 1);
	if (ret == FAIL)
		return 0;
	cnt = snprintf(tmp, TP_INFO_MAX_LENGTH, "0x%04x:0x%02x\n", fw_info->version, cfg_ver);
...
static const struct file_operations gtp_fw_version_ops = {
	.read		= gtp_fw_version_read,
};

static ssize_t gtp_lockdown_info_read(struct file *file, char __user *buf, size_t count, loff_t *pos)
{
	int cnt = 0, ret = 0;
	char tmp[TP_INFO_MAX_LENGTH];
	struct goodix_ts_data *ts = i2c_get_clientdata(i2c_connect_client);
...
static const struct file_operations gtp_lockdown_info_ops = {
	.read		= gtp_lockdown_info_read,
};
				    ...
				    struct device_attribute *attr, char *buf)
{
	...
	return scnprintf(buf, PAGE_SIZE, "GT%s_%x_%x\n",
...
static DEVICE_ATTR(panel_color, 0444, gtp_panel_color_show, NULL);
...
static DEVICE_ATTR(panel_vendor, 0444, gtp_panel_vendor_show, NULL);
...
static int gtp_create_file(struct goodix_ts_data *ts)
{
	...
	ts->tp_fw_version_proc = proc_create("tp_fw_version", 0, NULL, &gtp_fw_version_ops);
	ts->tp_lockdown_info_proc = proc_create("tp_lockdown_info", 0, NULL, &gtp_lockdown_info_ops);
...
static int gtp_request_irq(struct goodix_ts_data *ts)
{
	...
	/* use irq */
	if (gpio_is_valid(ts->pdata->irq_gpio) || ts->irq > 0) {
		if (gpio_is_valid(ts->pdata->irq_gpio))
...
static void gtp_switch_mode_work(struct work_struct *work)
{
	struct goodix_ts_mode_switch *ms = container_of(work, struct goodix_ts_mode_switch, switch_mode_work);
	struct goodix_ts_data *gtp = ms->data;
	u8 value = ms->mode;

	if (gtp && (value == GOODIX_INPUT_EVENT_WAKUP_MODE_ON || value == GOODIX_INPUT_EVENT_WAKUP_MODE_OFF)) {
		gtp->pdata->slide_wakeup = value - GOODIX_INPUT_EVENT_WAKUP_MODE_OFF;
...
{
	struct goodix_ts_data *gtp = input_get_drvdata(dev);
	struct goodix_ts_mode_switch *ms;

	if (type == EV_SYN && code == SYN_CONFIG) {
		if (test_bit(SLEEP_MODE, &gtp->flags)) {
			dev_info(&gtp->client->dev, "don't receive dbclick cmd in suspend state\n");
			return 0;
		}
		dev_info(&gtp->client->dev, "event write value = %d\n", value);

		if (value >= GOODIX_INPUT_EVENT_START && value <= GOODIX_INPUT_EVENT_END) {
			ms = (struct goodix_ts_mode_switch *)kmalloc(sizeof(struct goodix_ts_mode_switch), GFP_ATOMIC);
			if (ms != NULL) {
				ms->data = gtp;
				ms->mode = (u8)value;
				INIT_WORK(&ms->switch_mode_work, gtp_switch_mode_work);
				schedule_work(&ms->switch_mode_work);
			} else {
				dev_err(&gtp->client->dev, "Failed in allocating memory for gtp_mode_switch!\n");
				return -ENOMEM;
			}
		}
	}
...
static s8 gtp_request_input_dev(struct goodix_ts_data *ts)
{
	...
	if (!ts->pdata->type_a_report) {
		input_mt_init_slots(ts->input_dev, ts->pdata->max_touch_id, INPUT_MT_DIRECT);
			...
			struct goodix_ts_platform_data *pdata)
{
	...
	pdata->use_default_cfg = of_property_read_bool(np, "goodix,use-default-cfg");
	of_property_read_u32(np, "goodix,default-sensor-id",
			     &pdata->default_sensor_id);
	...
	for_each_child_of_node(np, sub_np) {
		ret = of_property_read_u32(sub_np, "goodix,vendor", &temp_val);
		if (ret) {
			dev_err(dev, "can't get vendor id\n");
		} else {
			pdata->cfg_info[i].vendor = (u8)temp_val;
			dev_info(dev, "vendor id:0x:%x\n", pdata->cfg_info[i].vendor);
		}
		ret = of_property_read_string(sub_np, "goodix,cfg-name",
 ...
 *		0-succeed, -1-faileds.
 *******************************************************/
int gtp_parse_dt_cfg(struct device *dev, u8 *cfg, int *cfg_len, char *cfg_name)
{
	struct device_node *np = dev->of_node;
	struct property *prop;
	int ret;
...
#ifdef CONFIG_TOUCHSCREEN_GT9XX_DEBUG_FS
static void tpdbg_suspend(struct goodix_ts_data *ts, bool enable)
{
	if (enable)
		gtp_suspend(ts);
	else
...
{
	struct goodix_ts_data *ts = file->private_data;
...
static int gtp_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	dev_err(&client->dev, "GTP Driver Version: %s\n", GTP_DRIVER_VERSION);
	...
	ret = gtp_get_lockdown_info(ts);
	if (ret != SUCCESS)
		dev_err(&client->dev, "get lockdown info err\n");
	else {
		tp_maker = kzalloc(20, GFP_KERNEL);
		if (tp_maker == NULL)
			dev_err(&client->dev, "fail to alloc vendor name memory\n");
		else {
			kfree(tp_maker);
			tp_maker = NULL;
		}
	...
	if (ret) {
		dev_info(&client->dev, "symlink sysfs fail");
	}
	...
	if (ts->debugfs)
		debugfs_create_file("switch_state", 0600, ts->debugfs, ts, &tpdbg_operations);
	ts->tpfs = debugfs_create_dir("touch", NULL);
	if (ts->tpfs)
		debugfs_create_file("tp_ic_vendor", 0400, ts->tpfs, ts, &tp_ic_vendor_operations);
#endif
#ifdef CONFIG_TOUCHSCREEN_GT9XX_SELFTEST
	gtp_test_sysfs_init();
#endif
...
static int gtp_drv_remove(struct i2c_client *client)
{
	...
	if (ts->tp_lockdown_info_proc)
		proc_remove(ts->tp_lockdown_info_proc);
...
static int gtp_pm_suspend(struct device *dev)
{
	...
	if (ts->pdata->slide_wakeup) {
		dev_info(&ts->client->dev, "touch enable irq wake\n");
...
static int gtp_pm_resume(struct device *dev)
{
	...
	if (ts->pdata->slide_wakeup) {
		dev_info(&ts->client->dev, "touch disable irq by wake");
