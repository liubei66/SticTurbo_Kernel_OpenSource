...
#define HAP_VMAX_MAX_MV_USER	3596
#define HAP_MIN_TIME_STRONG		3596
#define HAP_MIN_TIME_CALL		3596
	...
	if (!rc) {
		...
		if (!prop) {
				...
				for (temp = 0; temp < chip->effect_max; temp++) {
					pr_debug("effect_arry:%u: %u,%u,%u,%u,%u,%u,%u,%u\n",
...
static int qpnp_haptics_parse_dt(struct hap_chip *chip)
{
	...
	if (!rc) {
		...
		if (rc == -EINVAL && chip->act_type == HAP_LRA) {
			pr_debug("Play mode not specified, using auto mode\n");
