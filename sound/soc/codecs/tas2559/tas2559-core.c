...
int tas2559_update_edge(struct tas2559_priv *pTAS2559, enum channel dev)
{
	int nResult = 0;

	dev_dbg(pTAS2559->dev,
		"%s, dev: %d, left edge: %d, right edge : %d\n",
		__func__, dev, pTAS2559->mnLeftEdge, pTAS2559->mnRightEdge);

	if (dev & DevA) {
		nResult = pTAS2559->update_bits(pTAS2559, DevA, TAS2559_SPK_CTRL_REG, 0x7, pTAS2559->mnLeftEdge);
		if (nResult < 0)
			goto end;
	}

	if (dev & DevB)
		nResult = pTAS2559->update_bits(pTAS2559, DevB, TAS2560_LOAD, 0xe0, pTAS2559->mnRightEdge << 5);
...
void tas2559_fw_ready(const struct firmware *pFW, void *pContext)
{
	...
	if (unlikely(!pFW) || unlikely(!pFW->data)) {
		dev_err(pTAS2559->dev, "firmware is not loaded.\n");
		goto end;
	}
...
int tas2559_parse_dt(struct device *dev, struct tas2559_priv *pTAS2559)
{
	...
	pTAS2559->spk_id_gpio_p = of_parse_phandle(np,
					"ti,spk-id-pin", 0);
	if (!pTAS2559->spk_id_gpio_p) {
		dev_dbg(pTAS2559->dev, "property %s not detected in node %s",
		"ti,spk-id-pin", np->full_name);
		pTAS2559->mnSpkType = VENDOR_ID_AAC;
	} else {
		pTAS2559->mnSpkType = spk_id_get(pTAS2559->spk_id_gpio_p);
	}

	dev_dbg(pTAS2559->dev, "spk is %d", pTAS2559->mnSpkType);
