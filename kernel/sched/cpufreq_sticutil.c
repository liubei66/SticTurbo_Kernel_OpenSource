 ...
 * @sg_policy: sticutil policy object to compute the new frequency for.
...
static void sugov_irq_work(struct irq_work *irq_work)
{
	...
	/*
	 * For RT and deadline tasks, the sticutil governor shoots the
	 * frequency to maximum. Special care must be taken to ensure that this
	 * kthread doesn't result in the same behavior.
	 *
	 * This is (mostly) guaranteed by the work_in_progress flag. The flag is
	 * updated only at the end of the sugov_work() function and before that
	 * the sticutil governor rejects all other frequency scaling requests.
	 *
...
static ssize_t sticboost_show(struct gov_attr_set *attr_set, char *buf)
{
	struct sugov_tunables *tunables = to_sugov_tunables(attr_set);
...
static ssize_t sticboost_store(struct gov_attr_set *attr_set, const char *buf,
				   size_t count)
{
	struct sugov_tunables *tunables = to_sugov_tunables(attr_set);

	if (kstrtobool(buf, &tunables->sticboost))
		return -EINVAL;
	return count;
}
...
static struct governor_attr sticboost = __ATTR_RW(sticboost);
...
static struct cpufreq_governor sticutil_gov;
		...
		struct sugov_tunables *tunables)
{
	...
	cached->sticboost = tunables->sticboost;
...
static void sugov_tunables_restore(struct cpufreq_policy *policy)
{
	...
	tunables->sticboost = cached->sticboost;
...
static int sugov_init(struct cpufreq_policy *policy)
{
	 ...
	 */
	tunables->up_rate_limit_us = CONFIG_STICUTIL_UP_RATE_LIMIT;
	tunables->down_rate_limit_us = CONFIG_STICUTIL_DOWN_RATE_LIMIT;
...
#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_STICUTIL
...
{
	return cpufreq_register_governor(&sticutil_gov);
}
