 ...
 * @sg_policy: realmeutil policy object to compute the new frequency for.
...
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_REALMEUTIL
static
#endif
struct cpufreq_governor cpufreq_gov_realmeutil;
...
static int cpufreq_realmeutil_cb(struct cpufreq_policy *policy,
				unsigned int event)
{
	switch(event) {
...
struct cpufreq_governor cpufreq_gov_realmeutil = {
	.name = "realmeutil",
	.governor = cpufreq_realmeutil_cb,
	.owner = THIS_MODULE,
};
static int __init sugov_register(void)
{
	return cpufreq_register_governor(&cpufreq_gov_realmeutil);
}
